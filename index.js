/**
  5 march 2020 remove premium of 306508548651745282
*/
  ...
  guildSchema.findOne({guild:role.guild.id}).then(async (guildData) => {
    if(guildData.ai){
      ...
      if (roleCreate.has(userId)) {
        let member = role.guild.members.get(userId);
        if (!member) return;
...
function raider(guild, member) {
  guild.owner.send(`${member}(${member.user.tag}) is a possible raider, I'm going to take action based on the permissions you gaved me. For max security give me administrator permission! You can disable this by setting \`setai\` to false.`);
...
client.on('guildMemberAdd', member => {
  guildSchema.findOne({ guild: member.guild.id }, function (err, guildData) {
    ...
    if (guildData) {
      if (guildData.join) {
        let channel = client.channels.get(guildData.join.channel);
        if (channel) {
          ...
          channel.send(msg).catch(() => { guildData.join = {}; save(guildData, member) });
        }
        ...
        if (autoroles) {
          if (autoroles.length != 0) {
            ...
            for (let i = 0; i < autoroles.length; i++) {
              let role = member.guild.roles.get(autoroles[i]);
...
client.on("guildMemberRemove", member => {
  guildSchema.findOne({ guild: member.guild.id }, function (err, guildData) {
    if (guildData) {
      if (guildData.leave) {
        let channel = client.channels.get(guildData.leave.channel);
        if (channel) {
          ...
          msg = msg.replaceAll('{username}', `${member.user.username}`);
          msg = msg.replaceAll('{nickname}', `${member.nickname ? member.nickname : member.user.username}`);
          channel.send(msg).catch(() => { guildData.leave = {}; save(guildData, member) });
        }
      }
    }
  })
...
client.on("messageDelete", message => {
  reactionSchema.deleteMany({ message: message.id }, function (err) { if (err) console.log(err) });
})
client.on("messageDeleteBulk", async function (messages) {
  await messages.array().reverse().forEach(message => {
    reactionSchema.deleteMany({ message: message.id }, function (err) { if (err) console.log(err) });
  })
});
client.on("messageReactionAdd", (reaction, user) => {
  let emoji = reaction._emoji.id == null ? reaction._emoji.name : reaction._emoji.id;
  reactionSchema.findOne({ guild: reaction.message.guild.id, message: reaction.message.id, reaction: emoji }, function (err, guildData) {
    if (guildData) {
      reaction.message.guild.roles.fetch(guildData.role).then(role => {
...
client.on('ready', async () => {
  client.user.setActivity(`${config.status} | ${config.version}`, { type: "PLAYING" });
    ...
    if (guildData) {
      if (guildData.timers) {
        guildData.timers.forEach(timer => {
          ...
          if (when < Date.now()) {
            timer.last = Date.now();
            var chan = client.channels.cache.get(timer.channel);
      ...
      guildData = { guild: message.guild.id, cc: [], timers: [], autorole: [], verify: {}, join: {}, leave: {}, prefix: "/", premium: false, inviteCode: null };
  ...
  joinDataSchema.find({ guild: member.guild.id }).then(guildData => {
    ...
    if (count.length > 1) {
      count.forEach(counter => {
        joinDataSchema.findByIdAndDelete(counter._id, function (err, guildData) { if (err) { console.log(err) } });
      })
...
function createCaptcha(member, verify) {
  if (!verify) return;
  client.checkPermission("MANAGE_ROLES", member);
  if (!member.manageable) return;
  var captcha = captchagen.create();
  var code = captcha.options.text;
  captcha.generate();
  ...
  member.send(embed).then(sentEmbed => {
    ...
    collector.on('collect', async collected => {
      if (collected.content !== code) {
        ...
        let memberToRole = guild.members.get(member.id);
...
function createChannelCaptcha(member, verify) {
  ...
  var embed = new discord.MessageEmbed()
    ...
    .setImage("attachment://captcha.png")
    .setFooter(config.footer)
  client.channels.get(verify.channel).send(embed).then(sentEmbed => {
    const collector = sentEmbed.channel.createMessageCollector(m => !m.author.bot, { time: 150000 });
    collector.on('collect', async collected => {
      collected.delete();
      if (collected.content !== code) {
        return client.channels.get(verify.channel).send("Incorrect captcha, please try again!").then(sentMessage => setTimeout(() => { sentMessage.delete() }, 5000));
      } else if (collected.content == code) {
        collector.stop("solved");
        client.channels.get(verify.channel).send("Captcha correct! You're verified").then(sentMessage => setTimeout(() => { sentMessage.delete() }, 5000));
        let guild = client.guilds.get(member.guild.id).catch(err => logcatch(err, member));
        let memberToRole = guild.members.get(member.id).catch(err => logcatch(err, member));
