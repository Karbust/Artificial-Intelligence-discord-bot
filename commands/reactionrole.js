    ...
    message.channel.send(`Please type \`${prefix}role <roleName> (without <>)\` to enter the role you want. Or type \`${prefix}role stop\` to save the entered values.`).then(msg1 => {
      ...
      message.channel.awaitMessages(filter, { max: 1, time: 60000, errors: ['time'] }).then(collected => {
        if(count >= 10){
            ...
            var content = collected.first().content.slice(prefix.length + 5);
        }
        if(content == "stop"){
          ...
          count++
          let role = message.guild.roles.find(r => r.name.toLowerCase() == content.toLowerCase());
          if(role){
            ...
            message.channel.send(`Please type \`${prefix}emoji <:emojiname:> (without <>)\` to enter the emoji you want for \`${role.name}\`.`).then(msg2 => {
              ...
              message.channel.awaitMessages(filter, { max: 1, time: 60000, errors: ['time'] }).then(collected => {
                ...
                if(emoji){
                  ...
                  reactionSchema.deleteMany({message: message.id, guild: message.guild.id}, function (err){if(err)console.log(err)});
                  return message.channel.send("Invalid emoji. Cancelling command!");
                }
              }).catch(collected => {if(collected.size ==0){reactionSchema.deleteMany({message: message.id, guild: message.guild.id}, function (err){if(err)console.log(err)}); return message.channel.send("No response after 1m. Cancelling command")}});
            });
          } else {
            reactionSchema.deleteMany({message: message.id, guild: message.guild.id}, function (err){if(err)console.log(err)});
