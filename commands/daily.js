                    ...
                    userSchema.findOne({user: message.author.id}, function (err, userData) {
                        ...
                        if(userData){
                           if(userData.lastdaily > Date.now() - '43200000'){
                                ...
                                return sendMSG.sendError(message.channel,`TimeLimit: You already claimed this! (you can vote again at ${prettyMilliseconds('43200000' - (Date.now() - userData.lastdaily))})`);
                           }
                        } else {
                          var userData = {user: message.author.id, votes: 0};
                        }
