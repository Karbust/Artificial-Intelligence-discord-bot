...
module.exports.run = async (prefix, client, message, args, id) => {
	if (!args[0]) {
		if (!message.channel.permissionsFor(message.guild.me).has('ADD_REACTIONS'))
			return sendMSG.sendError(message.channel, "It seems like I'm missing `ADD REACTIONS` permission! PLease give me this permission.");
		...
		client.cmdhelp
			...
			.forEach((cmd) => {
				funArr.push(cmd.name);
			});
		client.cmdhelp
			.filter((cmd) => cmd.category === 'Custom commands')
			.forEach((cmd) => {
				ccArr.push(cmd.name);
			});
		client.cmdhelp
			.filter((cmd) => cmd.category === 'Timers')
			.forEach((cmd) => {
				timersArr.push(cmd.name);
			});
		client.cmdhelp
			.filter((cmd) => cmd.category === 'Welcome commands')
			.forEach((cmd) => {
				welcomeArr.push(cmd.name);
			});
		client.cmdhelp
			.filter((cmd) => cmd.category === 'Roles')
			.forEach((cmd) => {
				roleArr.push(cmd.name);
			});
		client.cmdhelp
			.filter((cmd) => cmd.category === 'Economy')
			.forEach((cmd) => {
				ecoArr.push(cmd.name);
			});
		const pages = {
		...
		sendMSG
			...
			.then((m) => {
				...
				collector.on('collect', async (reaction) => {
					...
					let embed = new discord.MessageEmbed().setColor(config.default.embed.color).setFooter(config.footer);
					if (chosen === '◀') {
						if (page != min) {
		...
		sendMSG.sendEmbed(
			...
			cmd.description,
			[{name: 'Usage', value: '```' + cmd.usage + '```'}, {name: '\u200b', value: 'Use `help usage` to see all the usage parameters (<parameter>).'}],
			[],
			false
		);
		succeeded(id);
