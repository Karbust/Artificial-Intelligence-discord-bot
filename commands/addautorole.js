...
module.exports.run = async (prefix, client, message, args, id) => {
	if (!message.member.permissions.has('ADMINISTRATOR') && !config.whitelist.includes(message.author.id)) return sendMSG.sendError("PermissionError: You haven't enough permission to do this");
	...
	guildSchema.findOne({guild: message.guild.id}).then((guildData) => {
		...
		if ((guildData.premium === false && guildData.autorole.length >= 5) || guildData.autorole.length >= 100)
			return sendMSG.sendError(message.channel, 'PremiumError: Maximum autoroles reached. Try to remove others');
