...
module.exports.run = async (prefix, client, message, args, id) => {
  if(!message.member.permissions.has("MANAGE_CHANNELS") && !config.whitelist.includes(message.author.id)) return sendMSG.sendError(message.channel,"PermissionError: You haven't enough permission to do this");
  ...
  message.guild.channels.create("Private-room",{permissionOverwrites:perms}).then(channel => sendMSG.sendError(message.channel,`Created ${channel}`)).catch(err => logcatch(err,message));
  succeeded(id);
};
exports.help = {
  name: "openroom",
